/**
 * @file bench_bignum_jacobi_mt.c
 * @brief ISO C entry point for the multi-thread bignum_jacobi benchmark.
 *
 * @details
...
int main(int argc, char **argv)
{
    ...
    bignum_jacobi_benchmark_status_t adapter_status;
    benchmark_core_status_t core_status;

    adapter_status = bignum_jacobi_benchmark_adapter_init(&adapter);
    if (adapter_status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return EXIT_FAILURE;
    }
