/**
 * @file bignum_jacobi_benchmark_adapter.c
 * @brief Deterministic bignum_jacobi adapter implementation for benchmark-core.
 *
 * @details
 ...
 */
#include "bignum_jacobi_benchmark_adapter.h"
...
#define BIGNUM_JACOBI_BENCHMARK_FNV_OFFSET UINT64_C(1469598103934665603)
/** @brief Stable FNV-1a prime used for observable benchmark checksums. */
#define BIGNUM_JACOBI_BENCHMARK_FNV_PRIME UINT64_C(1099511628211)

/**
 * @brief Enumerates bignum Jacobi paths transported as `operation_kind`.
 *
 * @details
 * Each value maps one-to-one to the documented `jacobi-*` vocabulary. The
 * value is private to this adapter; generic benchmark-core sees only text.
 */
typedef enum {
    BIGNUM_JACOBI_JACOBI_FIXED = 0 /**< Evaluate each generated numerator against the fixed odd modulus. */
} bignum_jacobi_jacobi_kind_t;
 ...
 * @return A named bignum_jacobi_benchmark_status_t result.
 *
 * @details
 ...
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_string_equals(
    const char *left,
    const char *right,
    benchmark_boolean_t *equal)
{
    if (left == NULL || right == NULL || equal == NULL) {
        return BIGNUM_JACOBI_BENCHMARK_STATUS_NULL_ARGUMENT;
    }
    *equal = strcmp(left, right) == 0
        ? BENCHMARK_BOOLEAN_TRUE
        : BENCHMARK_BOOLEAN_FALSE;
    return BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS;
}
 ...
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_value_is_allowed(
    const char *value,
    ...
    benchmark_boolean_t *accepted)
{
    ...
    *accepted = BENCHMARK_BOOLEAN_FALSE;
    for (size_t index = 0U; allowed[index] != NULL; ++index) {
        benchmark_boolean_t equal;
        bignum_jacobi_benchmark_status_t status;

        status = bignum_jacobi_benchmark_string_equals(value, allowed[index], &equal);
        if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
            return status;
        }
 ...
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_validate_axis(
    const char *value,
    const char *const *allowed)
{
    benchmark_boolean_t accepted;
    bignum_jacobi_benchmark_status_t status =
        bignum_jacobi_benchmark_value_is_allowed(value, allowed, &accepted);

    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    return accepted == BENCHMARK_BOOLEAN_TRUE
        ? BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS
        : BIGNUM_JACOBI_BENCHMARK_STATUS_INVALID_PROFILE;
}

/**
 * @brief Map a generic `operation_kind` transport string to a bignum Jacobi kind.
 * @param[in] operation_kind Generic framework transport value.
 * @param[out] jacobi_kind Receives the corresponding bignum Jacobi path.
 * @return A named bignum_jacobi_benchmark_status_t result.
 *
 * @details
 * The mapping deliberately accepts only `jacobi-*` values. This prevents a
 ...
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_parse_jacobi_kind(
    const char *operation_kind,
    bignum_jacobi_jacobi_kind_t *jacobi_kind)
{
    if (operation_kind == NULL || jacobi_kind == NULL) {
        return BIGNUM_JACOBI_BENCHMARK_STATUS_NULL_ARGUMENT;
    }
    if (strcmp(operation_kind, "jacobi-fixed") == 0) {
        *jacobi_kind = BIGNUM_JACOBI_JACOBI_FIXED;
    } else {
        return BIGNUM_JACOBI_BENCHMARK_STATUS_INVALID_PROFILE;
    }
    return BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS;
}

/**
 * @brief Advance a deterministic xorJacobi state.
 * @param[in,out] state Non-zero mutable pseudo-random state.
 * @param[out] value Receives the next deterministic value.
 * @return A named bignum_jacobi_benchmark_status_t result.
 *
 * @details
 * The algorithm uses three reversible xorJacobi steps. A zero incoming seed is
 * normalized to a fixed non-zero constant so every valid workload creates a
 * deterministic sequence and never remains in the all-zero xorJacobi state.
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_next_value(
    uint64_t *state,
    uint64_t *value)
{
 ...
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_leading_zero_bits(
    uint64_t value,
    size_t *count)
{
    if (count == NULL) {
...
/**
 * @brief Compute the maximum left Jacobi that remains inside bignum capacity.
 * @param[in] number Valid normalized bignum source record.
 * @param[out] maximum Receives a non-overflowing Jacobi bound.
 * @return A named bignum_jacobi_benchmark_status_t result.
 *
 * @details
 * For zero values any Jacobi below the complete storage width is safe. For a
 ...
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_max_safe_shift(
    ...
    size_t *maximum)
{
    ...
    status = bignum_jacobi_benchmark_leading_zero_bits(
 ...
 * of capacity, leaving safe room for non-overflowing Jacobi workloads.
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_choose_length(
    ...
    size_t *length)
{
    ...
    if (strcmp(workload->size_profile, "one") == 0) {
        ...
        *length = BIGNUM_CAPACITY / 2U == 0U ? 1U : BIGNUM_CAPACITY / 2U;
        ...
        status = bignum_jacobi_benchmark_next_value(state, &random_value);
        if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
            return status;
        }
 ...
 * scheduling.
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_row_is_zero(
 ...
 *
 * @details
 * The callback validates profile fields, creates a row-specific xorJacobi seed,
 ...
 */
static benchmark_adapter_status_t bignum_jacobi_benchmark_initialize(
    void *state,
    uint64_t sequence_index,
    const benchmark_workload_t *workload,
    void *adapter_context)
{
    ...
    status = bignum_jacobi_benchmark_validate_workload(workload);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
    }
    memset(number, 0, sizeof(*number));
    status = bignum_jacobi_benchmark_row_is_zero(workload->input_kind, sequence_index, &zero);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
    }
    ...
    status = bignum_jacobi_benchmark_choose_length(workload, &random_state, &length);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS || length == 0U || length > BIGNUM_CAPACITY) {
        return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
    }
    number->len = length;
    for (size_t word = 0U; word < length; ++word) {
        status = bignum_jacobi_benchmark_next_value(&random_state, &random_value);
        if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
            return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
        }
...
/**
 * @brief Choose one representable bignum left-Jacobi amount for an operation callback.
 * @param[in] number Immutable source state copied by benchmark-core.
 * @param[in] jacobi_kind Parsed bignum Jacobi category.
 * @param[in] iteration Logical adapter operation iteration.
 * @param[in] seed Workload seed used to derive deterministic choices.
 * @param[out] jacobi_amount Receives a representable non-negative Jacobi amount.
 * @return A named bignum_jacobi_benchmark_status_t result.
 *
 * @details
 ...
 */
static bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_choose_shift(
    const bignum_t *number,
    bignum_jacobi_jacobi_kind_t jacobi_kind,
    uint64_t iteration,
    uint64_t seed,
    size_t *jacobi_amount)
{
    ...
    if (number == NULL || jacobi_amount == NULL) {
        return BIGNUM_JACOBI_BENCHMARK_STATUS_NULL_ARGUMENT;
    }
    status = bignum_jacobi_benchmark_max_safe_shift(number, &maximum);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    if (number->len == 0U || jacobi_kind == BIGNUM_JACOBI_JACOBI_FIXED || maximum == 0U) {
        *jacobi_amount = 0U;
        return BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS;
    }
    if (jacobi_kind == BIGNUM_JACOBI_JACOBI_FIXED) {
        jacobi_kind = (bignum_jacobi_jacobi_kind_t)(iteration % UINT64_C(4));
    }
    status = bignum_jacobi_benchmark_next_value(&random_state, &random_value);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return status;
    ...
    if (jacobi_kind == BIGNUM_JACOBI_JACOBI_FIXED) {
        *jacobi_amount = 1U + (size_t)(random_value % (maximum < 63U ? maximum : 63U));
    } else if (jacobi_kind == BIGNUM_JACOBI_JACOBI_FIXED) {
        *jacobi_amount = maximum / 64U == 0U ? 0U :
            64U * (1U + (size_t)(random_value % (maximum / 64U)));
    } else if (jacobi_kind == BIGNUM_JACOBI_JACOBI_FIXED) {
        const size_t words = maximum / 64U;
        *jacobi_amount = words == 0U ? (maximum < 2U ? 0U : 1U) :
            64U + 1U + (size_t)(random_value % (maximum - 64U));
    } else {
        *jacobi_amount = (size_t)(random_value % (maximum + 1U));
    }
    return BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS;
}

/**
 * @brief Execute bignum_jacobi once on a benchmark-core mutable state copy.
 ...
 *
 * @details
 * The callback maps `jacobi-*` text to a domain enum, derives a safe deterministic
 * amount, invokes bignum_jacobi in-place, and maps its named library status
 * to benchmark-core's named callback result.
 */
static benchmark_adapter_status_t bignum_jacobi_benchmark_operation(
    void *state,
    uint64_t iteration,
    const benchmark_workload_t *workload,
    void *adapter_context)
{
    ...
    size_t jacobi_amount;
    bignum_jacobi_benchmark_status_t status;
    bignum_jacobi_status_t operation_status;
    bignum_t modulus = {{UINT64_C(65537)}, 1U};
    int symbol = 0;
    ...
    status = bignum_jacobi_benchmark_parse_jacobi_kind(workload->operation_kind, &jacobi_kind);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
    }
    status = bignum_jacobi_benchmark_choose_shift(
        number, jacobi_kind, iteration, workload->seed, &jacobi_amount);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
    }
    (void)jacobi_amount;
    operation_status = bignum_jacobi(number, &modulus, &symbol);
    return operation_status == BIGNUM_JACOBI_SUCCESS
 ...
 */
static uint64_t bignum_jacobi_benchmark_checksum(
    const void *state,
    uint64_t iteration,
    void *adapter_context)
{
    const bignum_t *number = state;
    uint64_t checksum = BIGNUM_JACOBI_BENCHMARK_FNV_OFFSET;
    ...
    for (size_t word = 0U; word < BIGNUM_CAPACITY; ++word) {
        ...
        checksum *= BIGNUM_JACOBI_BENCHMARK_FNV_PRIME;
    }
...
bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_validate_workload(
    const benchmark_workload_t *workload)
{
    ...
    status = bignum_jacobi_benchmark_validate_axis(workload->input_kind, input_values);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    status = bignum_jacobi_benchmark_validate_axis(workload->operation_kind, operation_values);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    status = bignum_jacobi_benchmark_validate_axis(workload->measure_mode, measure_values);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    status = bignum_jacobi_benchmark_validate_axis(workload->size_profile, size_values);
    if (status != BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    return bignum_jacobi_benchmark_validate_axis(workload->capacity_profile, capacity_values);
}

bignum_jacobi_benchmark_status_t bignum_jacobi_benchmark_adapter_init(
    benchmark_adapter_t *adapter)
{
    ...
    *adapter = (benchmark_adapter_t){
        .benchmark_name = "bignum_jacobi",
        .state_size = sizeof(bignum_t),
        .success_code = BENCHMARK_ADAPTER_STATUS_SUCCESS,
        .adapter_context = NULL,
        .initialize = bignum_jacobi_benchmark_initialize,
        .operation = bignum_jacobi_benchmark_operation,
        .checksum = bignum_jacobi_benchmark_checksum
    };
    return BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS;
}
