/**
 * @file bignum_jacobi_benchmark_adapter.h
 * @brief C11 domain adapter between bignum_jacobi and benchmark-core.
 *
 * @details
 ...
 * in-place bignum_jacobi operation.
 *
 ...
 * `jacobi-zero`, `jacobi-bit`, `jacobi-word`, `jacobi-combined`, `jacobi-random`,
 * or `jacobi-mixed`; the adapter removes the `jacobi-` transport prefix and
 * applies the resulting bignum Jacobi path. benchmark-core `size_profile`
 * accepts `one`, `quarter`, `half`, `variable`, or `near-capacity`; the
 * adapter uses it as the bignum operand-length profile.
 */
#ifndef BIGNUM_JACOBI_BENCHMARK_ADAPTER_H
#define BIGNUM_JACOBI_BENCHMARK_ADAPTER_H
...
/**
 * @brief Named outcomes returned by bignum_jacobi adapter construction and callbacks.
 *
 * @details
 ...
 * map invalid configuration and unsuccessful bignum_jacobi execution to the
 * corresponding benchmark_adapter_status_t expected by benchmark-core.
 */
typedef enum {
    BIGNUM_JACOBI_BENCHMARK_STATUS_SUCCESS = 0, /**< The requested adapter action completed successfully. */
    BIGNUM_JACOBI_BENCHMARK_STATUS_NULL_ARGUMENT = 1, /**< A required output, workload, state, or adapter pointer was NULL. */
    BIGNUM_JACOBI_BENCHMARK_STATUS_INVALID_PROFILE = 2, /**< A transport field is not a valid bignum profile value. */
    BIGNUM_JACOBI_BENCHMARK_STATUS_OPERATION_ERROR = 3 /**< bignum_jacobi rejected the generated in-place operation. */
} bignum_jacobi_benchmark_status_t;

/**
 * @brief Initialize a benchmark-core binding for the bignum_jacobi operation.
 ...
 * `jacobi-*` values, preserving the distinction between zero, bit, word,
 * combined, random, and mixed Jacobi paths. `size_profile` is restricted to
